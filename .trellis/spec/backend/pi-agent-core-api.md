...
显式 skill 调用入口（需 idle）。core 用 `formatSkillInvocation(skill, additionalInstructions?)` 拼 prompt 后走 `executeTurn`；`additionalInstructions` **原样追加**（无 args 时不传第二参数）。未知 name 抛 `Unknown skill`；busy 抛 `AgentHarnessError("busy")`。

Novi TUI 路由：`/skill:name [args]`（仅 TUI；优先于静态 COMMANDS 与 prompt-template fallback）。本地先查 `getResources().skills` 给出 Novi 风格 unknown/busy notice，再调 `harness.skill`。slash 补全展示 `skill:<name>` + description。

`Skill.disableModelInvocation`：loader 解析 frontmatter `disable-model-invocation: true`；`formatSkillsForSystemPrompt` 过滤此类 skill（不进 system prompt），但资源仍可加载，**显式 `/skill:name` 与补全仍可用**。
...
4. **skillsBlock**：`formatSkillsForSystemPrompt(resources.skills)`（core 自动过滤 `disableModelInvocation`）。

> **注意**：base 是**替换**（取第一个非空），appendBlock/contextBlock/skillsBlock 是**追加**。
> SYSTEM.md 替换默认 prompt 但保留 append + context + skills 拼接。
>
> Skills 发现路径（`loadResources` / `resolveSkillSources`）：`~/.agents/skills` → `~/.novi/skills` → git-root→cwd 各级 `.agents/skills` → `<cwd>/.novi/skills`（后写覆盖；用户层不 trust 门控；项目侧 trust 门控）。
