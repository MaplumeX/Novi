...
- [ ] AC-S1：P0 + P1 + P3 均有 child 实现与测试覆盖；parent 做最终集成评审。
- [ ] AC-S2：reaction / quote 独立模型 / 用户消息 edit 入站 / 出站媒体 / 出站强制 reply / P2 / P4 不出现在交付中。
- [ ] AC-I1：三 child 合并后 `npm run typecheck && npm test` 全绿。
- [ ] AC-I2：Telegram 文本网关与 durable delivery 现有行为回归无破坏。

### P0 Telegram media

- [ ] AC-M1：图片/文件/语音消息入站并归一化为统一附件模型（类型/mime/size/filename/路径）。
- [ ] AC-M2：图片进入 harness 多模态路径（`harness.prompt(text, { images })`）；无 vision 模型时给出 warning + 降级说明。
- [ ] AC-M3：文件/语音落盘，turn text 携带可操作路径与元数据。
- [ ] AC-M4：inbox 持久化记录不含媒体 blob（仅元数据/本地引用）。
- [ ] AC-M5：不支持的消息类型可诊断、不崩溃。

### P1 Feishu adapter

- [ ] AC-F1：飞书配置入 `gateway.json`，工厂可创建；`start/stop/probe` 闭环。
- [ ] AC-F2：入站文本 → `ChannelMessage` → session lane → 出站文本闭环。
- [ ] AC-F3：`ChannelCapabilities` 声明与 Telegram 有差异（edit-stream / threads），并真实反映飞书能力。
- [ ] AC-F4：与 allowlist/pairing/session route/durable delivery 兼容（既有测试套件不破）。
- [ ] AC-F5：P3 silent/thread/reply 在飞书侧行为与 Telegram 一致（按能力声明）。

### P3 Unified semantics

- [ ] AC-U1：silent markers + `cancelStream` 行为在 Telegram/飞书一致，有契约文档。
- [ ] AC-U2：thread 语义文档化；飞书 thread/topic 映射明确（支持或 `threads=false`）。
- [ ] AC-U3：`ChannelSendTarget.replyToMessageId?` 可选字段落地；不默认 reply；不破坏 edit-stream。
- [ ] AC-U4：bot 流式 edit 契约文档化（`capabilities.edit` + `sendEvent`）。
