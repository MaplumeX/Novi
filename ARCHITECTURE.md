...
### 4.5 resources.ts
加载 skills（`loadSourcedSkills`）与 prompt templates（`loadPromptTemplates`）。skills 按 name 去重，**后写覆盖**；prompt templates 不去重。加载器跳过非法文件并收集 diagnostics，绝不抛错。`loadResources` 接受 `{ includeProject }` —— untrusted 时只扫描用户层。

**Skills 发现顺序（D4，后写胜）**：
1. `~/.agents/skills`（user，不 trust 门控）
2. `~/.novi/skills`（user，不 trust 门控）
3. 从 git root → cwd 各级 `dir/.agents/skills`（project，trust 门控；非 git 只扫 cwd）
4. `<cwd>/.novi/skills`（project，trust 门控）

git root 用 env IO 探测 `.git`（file 或 dir），无 child_process。`findGitRoot` / `resolveSkillSources` 导出供 trust 与测试复用。

### 4.6 trust.ts
项目信任门，控制是否加载项目侧 gated 资源。持久化于 `~/.novi/trust.json`（0600），值为 `always`|`never`，`ask` 为默认不落盘。
...
- `hasGatedResources`：探测 `<cwd>/.novi/{settings,models,skills,prompts}` **或** git-root→cwd 上的 `.agents/skills`；`~/.agents/skills` 不算 gated。
...
| 文件 | 职责 |
| --- | --- |
| `encode.ts` | `encodeImageBytes` / `loadImageFile` / `appendPending`；mime 白名单 png/jpeg/gif/webp；单张 ≤10MiB；pending 最多 8 张 |
| `clipboard.ts` | `createClipboardImageReader()` — darwin `osascript` 写临时 PNG；linux `wl-paste`/`xclip`；其他平台明确错误 |

纯 backend 可测；TUI 通过 `CommandContext.addPendingImages` 与提交路径消费 `PendingImage`。

### 4.13 default-system-prompt.ts
...
└─ <App>
   ...
   └─ Overlay (互斥其一):
        ...
        ├─ <FilePicker>         @file 触发：选文件路径插入编辑器；`/image` 无参时 imagePicker 模式（扩展名过滤，选中附加）
...
- **Enter**：根据 phase 分流 `handlePrompt` (idle) / `handleSteer` (turn) / `handleFollowUp`，slash 行走 `handleCommand`。有 pending 图片时三者都传 `{ images }`；非 vision 模型提交前 print 警告仍发出。
- **Ctrl+I** / `/paste-image`：从系统剪贴板附加图片到 pending（不拦截原生文本粘贴）。
- **`/image`**：路径附加或打开仅图片扩展名的 file picker；`@` 仍只插入路径。
...
- 图片附件：`/image [path]`（无参打开图片 picker）、`/image clear`、`/paste-image`（等同 Ctrl+I）
...
| `InputBox.tsx` | 编辑器核心：cursor 模型、Emacs 键位、`@file` 触发、`!`/`!!` bang、Ctrl+G 外部编辑器、Ctrl+I 剪贴板图片、Tab 路径补全、pending 附件条 |
...
### 配置目录（两层级 + 共享 skills）
```
~/.agents/skills/                跨 harness 共享 skills（user，不 trust 门控；可被 ~/.novi 覆盖）
~/.novi/                         用户级（全局）
...
# 项目侧（trust 门控）：git root → cwd 各级 .agents/skills + <cwd>/.novi/*
<git-root…cwd>/.agents/skills/   共享项目 skills（近覆盖远）
<cwd>/.novi/                      项目级（覆盖全局；同名时 project 优于 user）
├── settings.json                 项目配置
├── SYSTEM.md
├── APPEND_SYSTEM.md
├── models.json                  项目自定义 providers（同 id 覆盖全局）
├── skills/                       覆盖同名 project .agents / user skills
├── prompts/
├── hooks/hooks.json             项目钩子 manifest
└── AGENTS.md                     上下文候选之一（父目录链 + cwd 聚合去重）
```

TUI 显式调用：`/skill:name [args]` → `harness.skill(name, args?)`（仅 idle；unknown/busy 打 notice）。slash 补全展示 `skill:<name>` + description。`disableModelInvocation` skill 仍可显式调用与补全，但不进 system prompt skills 块。
