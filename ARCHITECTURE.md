...
6. `createChannels` + `NoviAgentAdapter` + `GatewaySessionManager` + `GatewayApp`  
7. `SIGHUP`：仅 access / group-routing 热更；token/channel/queue 等需重启  
8. SIGINT/SIGTERM → `app.stop()`
...
*基于当前工作树编写。实现变更时请以 `src/**` 为准并回写本节链接目标。*
