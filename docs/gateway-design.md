...
此外，常驻进程还挂着 durable scheduled jobs、heartbeat、共享 AgentRunManager 与主动投递能力；它们与入站消息共用 session 路由与部分 harness 生命周期，但**不是**本文的主线。
...
5. `createChannels`，打开 `GatewaySessionStore` / `JobStore`，构造 `NoviAgentAdapter`、`GatewaySessionManager`、共享 `AgentRunManager` 与 command registry，并接线 `AutomationAgentRunner` / `DeliveryService` / `HeartbeatService` / `GatewayScheduler`。
...
同一进程还只有一个 `AgentRunManager`，所有 route 的 `agents` 工具共享全局并发。
child 完成后不会直接写 session 或调用 channel：completion 先进入所属 route 的
system-operation lane，按 generation 检查并去重 custom entry，再由父 harness 综合；
最终文本以 completion idempotency key 进入 durable outbox。`/new` 在 rotate 前取消
旧 generation，普通 `/stop` 只 abort 当前父 turn。详细合同见
`docs/agent-runs-design.md`。
