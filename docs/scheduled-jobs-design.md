...
prompt deadline、assistant usage 汇总与 UTF-8 结果截断由共享的
`executeHarnessPrompt` 完成，AgentRun executor 也复用这段骨架；两边仍各自负责
session 创建/清理、权限策略和 ledger 状态转换。Gateway 还把两类后台模型调用
接到同一个 `RunConcurrencyLimiter(automation.maxConcurrentLlmRuns)`，因此 jobs
预算与 AgentRun 全局队列仍分开，但 provider 同时在途量有统一上限。
...
Jobs **复用** route key、session binding、channel `send`、session manager 的 system operation、adapter 的 `appendScheduledDelivery`，并与 AgentRun 共享 harness execution primitive、bounded error 与 Gateway provider limiter。Jobs **不**走 `GatewayApp` 入站 dedupe/配对/queue mode，也不把 scheduled 输出再喂回入站 agent 循环；其 `$NOVI_HOME/jobs` schema、scheduledRunId、预算与投递状态机也不并入 AgentRun。理解 Gateway 时，应把 jobs 看成挂在同一进程上的第二条闭环，而不是 `onMessage` 的一个分支。
