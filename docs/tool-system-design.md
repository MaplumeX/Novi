...
`McpClientManager` 管理 server 连接与 `callTool`；`adaptMcpTools` 把 MCP `Tool` 变成 `source.kind === "external"` 的 `ToolDescriptor`：
...
- factory 内部调用 manager，错误同样走 `NOVI_ERROR:TOOL_EXECUTION_FAILED:...`。
